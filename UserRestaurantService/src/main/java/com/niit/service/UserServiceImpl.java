...
//        System.out.println(user.getDob());
//        String dob[] = user.getDob().split("T");
//        user.setDob(dob[0]);
        if (userRepo.findById(user.getEmail()).isPresent()) {
...
//        restaurantRepo.save(restaurant);
    ...
    public int cartItemNumber(String id){
        if(userRepo.findById(id).get().getCart() != null)
            return userRepo.findById(id).get().getCart().size();
        return 0;
    }

    @Override
    public boolean addedToCart(String id, String cuisineId){
        User user = userRepo.findById(id).get();
        List<Cuisine> cuisines = user.getCart();
        if(cuisines != null){
    ...
    public double getCartTotal(String id) {
        List<Cuisine> cuisines = userRepo.findById(id).get().getCart();
        double sum = 0;
        if (cuisines != null){
            for (Cuisine cuisine : cuisines) {
                sum += cuisine.getQuantity() * cuisine.getPrice();
            }
        return sum;
    }
        return 0;
    ...
    @Override
    public Address getAddressById(String id, String addressId) {
        List<Address> addresses=userRepo.findById(id).get().getAddresses();
